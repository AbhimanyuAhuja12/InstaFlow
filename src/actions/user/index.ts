        ...
        if(found){
            if(found.integrations.length>0){
                ...
                if(days< 5){
                    ...
                    // updating the dates after refreshing the refreshToken
                    const today = new Date()
                    const expire_date = today.setDate(today.getDate()  + 60)
                    const update_token = await updateIntegrations(refresh.access_token, new Date(expire_date), found.integrations[0].id)
